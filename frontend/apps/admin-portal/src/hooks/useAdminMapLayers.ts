...
import { useAdminDangerZones, useAdminCamps, useAdminSOS } from './useAdminData'
...
export function useAdminMapLayers(
  ...
  useEffect(() => {
    ...
    if (!map.getSource('admin-zones')) {
      map.addSource('admin-zones', { type: 'geojson', data: geojsonData })
      map.addLayer({
        id: 'admin-zones-fill',
        type: 'fill',
        source: 'admin-zones',
        paint: { 'fill-color': '#b91c1c', 'fill-opacity': 0.3 },
      })
      map.addLayer({
  ...
  useEffect(() => {
      ...
      camps.forEach((camp: any) => {
        ...
        const popup = new maplibregl.Popup({ offset: 15 }).setHTML(`
          <div style="font-family: inherit; padding: 4px; min-width: 140px;">
            ...
            <div style="font-size: 10px; font-weight: bold; text-transform: uppercase; color: #666; margin-bottom: 8px;">Status: ${camp.stock_status.replace('_', ' ')}</div>
            <button id="nav-btn-camp-${camp.id}" style="width: 100%; background: #1976d2; color: white; border: none; padding: 6px; border-radius: 4px; cursor: pointer; font-weight: 900; text-transform: uppercase; font-size: 10px;">View Details</button>
          </div>
        `)
        popup.on('open', () => {
          setTimeout(() => {
            const btn = document.getElementById(`nav-btn-camp-${camp.id}`)
            if (btn)
              btn.onclick = () =>
                navigate({ to: '/logistics/$id', params: { id: camp.id } })
          }, 0)
        })
    ...
    if (filters.sos && sosSignals?.data) {
      sosSignals.data.forEach((sos: any) => {
        ...
        const isResolved = sos.status === 'resolved' || sos.status === 'closed'
        if (isResolved && !filters.resolvedSos) return

        const el = document.createElement('div')
        if (isResolved) {
          el.className =
            'w-5 h-5 bg-gray-400 rounded-full border-2 border-white shadow-md cursor-pointer opacity-70'
          el.innerHTML = '✓'
        } else {
          el.className =
        ...
        const popup = new maplibregl.Popup({ offset: 15 }).setHTML(`
          <div style="font-family: inherit; padding: 4px; min-width: 140px;">
            <div style="font-weight: 900; text-transform: uppercase; font-size: 14px; color: ${isResolved ? '#6b7280' : '#b91c1c'}; margin-bottom: 4px;">${isResolved ? 'Resolved SOS' : 'SOS Signal'}</div>
            <div style="font-size: 10px; font-weight: bold; text-transform: uppercase; color: #666;">Status: ${sos.status}</div>
            <div style="font-size: 10px; font-weight: bold; text-transform: uppercase; color: #666; margin-bottom: 8px;">Battery: ${sos.battery_level}%</div>
            <button id="nav-btn-sos-${sos.id}" style="width: 100%; background: #1976d2; color: white; border: none; padding: 6px; border-radius: 4px; cursor: pointer; font-weight: 900; text-transform: uppercase; font-size: 10px;">View Details</button>
          </div>
        `)
        popup.on('open', () => {
          setTimeout(() => {
            const btn = document.getElementById(`nav-btn-sos-${sos.id}`)
            if (btn) btn.onclick = () => navigate({ to: '/sos' })
          }, 0)
        })
        const marker = new maplibregl.Marker({ element: el })
        ...
        el.addEventListener('click', () => {
          map.flyTo({
            center: [sos.location.lng, sos.location.lat],
            zoom: 15,
            duration: 1500,
          })
        })
  ...
  }, [camps, sosSignals, filters, isLoaded, mapRef, navigate])

  useEffect(() => {
    const map = mapRef.current
    if (!map || !isLoaded || !focusedEvent) return
    if (popupRef.current) popupRef.current.remove()
    map.flyTo({ center: focusedEvent.coords, zoom: 16, duration: 1500 })
    const popup = new maplibregl.Popup({ closeOnClick: false }).setLngLat(
      focusedEvent.coords,
    ).setHTML(`
        <div style="font-family: inherit; padding: 4px; min-width: 140px;">
          <div style="font-weight: 900; text-transform: uppercase; font-size: 14px; margin-bottom: 4px;">${focusedEvent.title}</div>
          <div style="font-size: 10px; font-weight: bold; text-transform: uppercase; color: #666; margin-bottom: 8px;">System Event Linked</div>
          <button id="nav-btn-feed-${focusedEvent.id}" style="width: 100%; background: #1976d2; color: white; border: none; padding: 6px; border-radius: 4px; cursor: pointer; font-weight: 900; text-transform: uppercase; font-size: 10px;">View Details</button>
        </div>
      `)
    popup.on('open', () => {
      setTimeout(() => {
        const btn = document.getElementById(`nav-btn-feed-${focusedEvent.id}`)
        if (btn) {
          btn.onclick = () => {
            if (focusedEvent.type.includes('SOS')) navigate({ to: '/sos' })
            else if (focusedEvent.type.includes('CAMP'))
              navigate({
                to: '/logistics/$id',
                params: { id: focusedEvent.id },
              })
  ...
  }, [focusedEvent, isLoaded, mapRef, navigate])
